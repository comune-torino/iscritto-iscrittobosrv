...
public class DomandaMaternaConverter extends AbstractConverter<it.csi.iscritto.serviscritto.dto.domanda.DomandaMaterna, Domanda> {

	@Override
	public Domanda convert(it.csi.iscritto.serviscritto.dto.domanda.DomandaMaterna source) {
		Domanda target = null;
		if (source != null) {
			target = new Domanda();
			...
			target.setElencoNidi(new DomandaConverters.MaternaConverter().convertAll(source.getElencoMaterne()));
